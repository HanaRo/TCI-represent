...
class TC_DUIdataset(Dataset):
    ...
    * mode_config: dict- the config of the dataset mode
            ...
            if 'select' in mode:
                ...
                for key in KEYS:
                    ...
                    if key not in self.select_config['select_config']:
                        ...
                        for pattern in self.select_config['select_config'][key]:
                            for _, row in self.segment_metadata.iterrows():
                                if row[key] == pattern:
                                    new_segment_metadata.append(row)
                            # Update the segment_metadata
            ...
            if 'remove' in mode:
                ...
                for key in KEYS:
                    ...
                    if key not in self.select_config['remove_config']:
                        ...
                        for pattern in self.select_config['remove_config'][key]:
                            for _, row in self.segment_metadata.iterrows():
                                if row[key] != pattern:
                                    new_segment_metadata.append(row)
                            # Update the segment_metadata
    ...
    def collate_fn(self, input_list):
        ...
        # pad the surrouding information to the max length
