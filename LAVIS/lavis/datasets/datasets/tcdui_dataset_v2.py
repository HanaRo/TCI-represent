...
class TCDUIDataset_v2(Dataset):
    ...
    * changing modality padding to be optional
    * if 'sur' is in the modality, length padding to max length and return a valid mask
    * update get item method to support different mode of dataset, including 'train', 'val', 'test'
    --------
    ...
    * mode_config: dict- the config of the dataset mode
        ...
        * length: int or None - the length of the dataset, each element should be a int, None means raw length
        ...
        enalble_modality_padding: bool - whether to enable modality padding, if True, the dataset will pad the missing modalities to the default shape
        max_sur_length: int - the maximum length of the surrounding information, if None, disable the length padding
    ...
    def __init__(self, data_root,
        ...
        if self.mode_config['mode'] == 'test':
            self._test_mode_init()

    def __len__(self):
        if self.mode_config['mode'] == 'test':
            return len(self.test_metadata)
        elif self.mode_config['length'] is None:
    ...
    def __getitem__(self, idx):
        if self.mode_config['mode'] == 'test':
            ...
            return self._getitem_normal(idx)
    ...
    def _getitem_normal(self, idx):
        ...
        if 'frames' in self.mode_config:
            ...
            img_frames = int(0.2 * frames)   # what about change to 0.2?
        else:
            frames = 100
            img_frames = 20
        ...
        for m in modality:
            if m == 'ego':
                ...
                data['sur'], data['sur_mask'] = self._get_sur_data(segment_metadata, start, end)
            elif m == 'img':
                data[m] = self._get_img_data(segment_metadata, start, end, img_frames)
            elif m == 'ins':
        ...
        # modality padding
        if self.mode_config.get('enable_modality_padding', False):
    ...
    def _getitem_test(self, idx):
        data = {}
        # get the segment metadata
        segment_metadata = self.test_metadata.iloc[idx]
        # locate the segment range
        start = segment_metadata['start']
        end = segment_metadata['end']
        # initialize the frame number
        if 'frames' in self.mode_config:
            frames = self.mode_config['frames']
            # WARNING: 0.21 is a magic number using in modalities padding, it should be changed to a config
            img_frames = int(0.2 * frames)
        else:
            frames = 100
            img_frames = 20
        # randomly select a subsequence
        if end - start < frames:
            # if the segment is shorter than frames, we can not select a subsequence
            success = False
        else:
            success = True
        # load multiple modalities
        ...
        for m in modality:
            if m == 'ego':
                ...
                data[m] = self._get_img_data(segment_metadata, start, end, img_frames)
                # data[m] = self._get_img_samples_data(idx)
            elif m == 'ins':
                data[m] = self._get_ins_data(segment_metadata, start, end)
            elif m == 'duig' or m == 'duir':
                data[m] = self._get_anno_data(m, segment_metadata, start, end)
    ...
    def _test_mode_init(self):
        '''
        Initialize the test mode, which is used to test the dataset.
        In this mode, the dataset will slice the all segment with a fixed size sliding window.
        Using a static config in current version, which is 100 frames with a stride of 10.
        '''
        self.test_metadata = pd.read_json('/'.join([self.data_root, 'test_metadata.json']), lines=True)
    ...
    #     """
    #     self.test_metadata = []
    #     self.test_metadata_ref = {}

    #     frames = self.mode_config.get('frames', 100)

    #     current_idx = 0
    #     for idx, metadata in self.segment_metadata.iterrows():
    ...
    #         end = metadata['end']
    #         if end - start < frames:
    #             # if the segment is shorter than the frames, add it to test_metadata as it is
    #             self.test_metadata.append(metadata.copy())
    #             current_idx += 1
    #         else:
    #             # slice the segment with a fixed size sliding window
    #             ## Update: change the stride to 10
    #             for i in range(start, end - frames + 1, 10):
    #                 test_segment_metadata = metadata.copy()
    #                 test_segment_metadata['start'] = i
    #                 test_segment_metadata['end'] = i + frames
    #                 self.test_metadata.append(test_segment_metadata)
    #                 current_idx += 1
    #         seg_key = f"{record}_{seg}"
    #         self.test_metadata_ref[seg_key] = [previous_idx, current_idx]
    #     # Form the test metadata DataFrame
    #     self.test_metadata = pd.DataFrame(self.test_metadata).reset_index(drop=True)            
    ...
    def _get_sur_data(self, segment_metadata, start, end) -> list:
        max_length = self.mode_config.get('max_sur_length', 11) # from the documentation
        # locate the segment metadata and metadata
        record = segment_metadata['record']
        metadata = self.metadata.loc[record]
        # load the sur data
        ...
        # new features in v2: length padding and masking
        padding_data, padding_mask = self._pad_and_shuffle(data, max_length, len(data[0][0]))
        return padding_data, padding_mask
    
    @staticmethod
    def _pad_and_shuffle(data, max_length, feature_dim):
        """
        Args:
        data: List[List[List[float]]]
            Outer list length B. Each element is a list of length L_i.
            Each inner element is a list/vector of length `feature_dim`.
        feature_dim: int

        Returns:
        padded_data: List[List[List[float]]]   # shape (B, L_max, feature_dim), still Python lists
        mask_tensor: torch.BoolTensor          # shape (B, L_max), mask of valid entries
        """
        L_max = max_length
        ...
        for frame in data:
            l = len(frame)
            # 1) pad the data (pure Python)
            frame_copy     = deepcopy(frame)
            padded_frame   = frame_copy + [zero_vec] * (L_max - l)
            # 2) create the mask list
            m = [True] * l + [False] * (L_max - l)

            # 3) shuffle with a single perm
            perm = list(range(L_max))
            random.shuffle(perm)

            padded_data.append([padded_frame[i] for i in perm])
            mask_lists.append([m[i]           for i in perm])

        # 4) convert mask_list to a BoolTensor
        mask_tensor = torch.tensor(mask_lists, dtype=torch.bool)  # (B, L_max)

        return padded_data, mask_tensor
    
    def _get_img_data(self, segment_metadata, start, end, img_frames) -> list:
        # locate the segment metadata and metadata
        record = segment_metadata['record']
        metadata = self.metadata.loc[record]
        ...
        frames = min(frames, img_frames)  # limit the frames to img_frames
        # load the img data
    ...
    def _get_img_samples_data(self, idx) -> torch.Tensor:
        data = None
        datapath = os.path.join(self.data_root, 'img_samples_pt', f'sample_{idx:05}.pt')
        if os.path.exists(datapath):
            data = torch.load(datapath, only_weight=True)
        else:
            raise FileNotFoundError(f"test_img_samples data {datapath} does not exist")
    ...
    def _frame_padding(self, data, frames, img_frames):
        for key, tensor in data.items():
            if key == 'ego':
                ...
                # new feature in v2: surrouding mask
                # default: (frames, words, feature_dim)
                length, word, channel = tensor.shape
                sur_mask = data['sur_mask']
                assert length < frames, f"length {length} should be less than frames {frames}"
                pad_length = frames - length
                pad_left = pad_length // 2
                pad_right = pad_length - pad_left
                left_pad = tensor[:1].repeat(pad_left, 1, 1)
                left_pad_mask = sur_mask[:1].repeat(pad_left, 1)
                right_pad = tensor[-1:].repeat(pad_right, 1, 1)
                right_pad_mask = sur_mask[-1:].repeat(pad_right, 1)
                data[key] = torch.cat([left_pad, tensor, right_pad], dim=0)
                data['sur_mask'] = torch.cat([left_pad_mask, sur_mask, right_pad_mask], dim=0)
            elif 'img' in key:
    ...
    def collater(self, input_list):
        ...
        # INFO: Update in v2: all surrouding data in samples in a batch should be equal to max length
        # INFO: Removing following code, since it is not needed anymore
...
DATASET = {
    'TCDUIdataset_v2': TCDUIDataset_v2,
}

if __name__ == '__main__':
    dataset = TCDUIDataset_v2(
