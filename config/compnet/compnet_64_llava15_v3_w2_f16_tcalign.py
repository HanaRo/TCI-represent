...
model_name = 'compnet_64_llava15_v3_w2_f16_tcalign'
...
model = CompNet(input_dim=32).to(device)  # Adjust input_dim as needed
...
def _loss_fn(status_order, t_score1, t_score2, c_score1, c_score2, comp_score1, comp_score2, anno_scores1, anno_scores2, delta=0.5):
    ...
    loss = c_loss + t_loss + comp_loss + tc_align_loss

    return loss, c_loss, t_loss, comp_loss, tc_align_loss
...
def training_step(model=model, train_loader=train_loader, optimizer=optimizer, scheduler=scheduler, epoch=None):
    ...
    for i, batch in enumerate(train_loader):
        ...
        loss, c_loss, t_loss, comp_loss, tcalign_loss = _loss_fn(status_order, t_score1, t_score2, c_score1, c_score2, comp_score1, comp_score2,
