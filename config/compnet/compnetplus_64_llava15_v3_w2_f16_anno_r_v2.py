...
from models import CompNetPlus
...
model_name = 'compnetplus_64_llava15_v3_w2_f16_anno_r_v2'
...
model = CompNetPlus(input_dim=16, hidden_dim=[64, 64], comp_num=[[0.01, 20.0]]).to(device)  # Adjust input_dim as needed
...
############### Training Step #############
def _loss_fn(status_order, t_score1, t_score2, c_score1, c_score2, comp_score1, comp_score2, anno_scores1, anno_scores2, delta=0.5):
    c_loss = torch.log(1 + torch.exp(status_order * (c_score1 - c_score2))).mean()
    # t_loss = torch.abs(t_score1 - t_score2).mean()
    # tc_align_loss = torch.relu(torch.abs(t_score1 - c_score1) - delta).mean() + torch.relu(torch.abs(t_score2 - c_score2) - delta).mean()
    comp_loss = torch.abs(comp_score1 - anno_scores1).mean() + torch.abs(comp_score2 - anno_scores2).mean()
    loss = c_loss + comp_loss

    return loss, c_loss, comp_loss
...
def training_step(model=model, train_loader=train_loader, optimizer=optimizer, scheduler=scheduler, epoch=None):
    ...
    for i, batch in enumerate(train_loader):
        ...
        t_input1 = t_mean1.to(device)
        t_input2 = t_mean2.to(device)
        c_input1 = c_mean1.to(device)
        c_input2 = c_mean2.to(device)
        ...
        loss, c_loss, comp_loss= _loss_fn(status_order, t_score1, t_score2, c_score1, c_score2, comp_score1, comp_score2,
                                                                torch.stack([anno_g1], dim=-1).to(device), torch.stack([anno_g2], dim=-1).to(device))
        ...
        # epoch_tcalign_loss.append(tcalign_loss.item())
    ...
    # epoch_tcalign_loss = sum(epoch_tcalign_loss) / len(epoch_tcalign_loss)
    ...
    # writer.add_scalar('train/tcalign_loss', epoch_tcalign_loss, epoch)
...
def validation_step(model=model, val_loader=val_loader, epoch=None):
    ...
    # writer.add_scalar('eval/tcalign_loss', epoch_tcalign_loss, epoch)
